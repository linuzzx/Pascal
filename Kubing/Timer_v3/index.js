...
function saveSolution() {
    ...
    sessionList[curSession].solutions[Object.keys(sessionList[curSession].solutions).length] = newSolution;
...
function updateStats() {
    let solArr = Object.values(sessionList[curSession].solutions).map(s => s.time);
    ...
    if (arr.length !== 0) {
        // timeList
        for (let s of Object.values(sessionList[curSession].solutions)) {
            let i = Object.values(sessionList[curSession].solutions).indexOf(s);
            let i5 = i - 4;
            let i12 = i - 11;
            let c = s.comment !== "" ? "*" : "&nbsp;";
...
function changePenalty(i) {
    ...
    sessionList[curSession].solutions[i].finalTime = p === -1 ? -1 : sessionList[curSession].solutions[i].time + p;
            ...
            if (confirm("Importing will override current data. Do you still want to import?")) {
                ...
                $.each(json, function(key, sessions) {
                    if (key.includes("session")) {
                        ...
                        const nSession = new Session(sessionId, sessionName,sessionRank,sessionScrType, Object.assign({}, sessionSolutions));
